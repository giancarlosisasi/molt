...
- **A cache keyed by identity of the thing, not the object.** The cache key is `(host, kind, repository, id)`, so a commit or a pull request is fetched once per release no matter how many changelog lines reference it.
- **Rate limits, retries, and backoff.** Molt honors `Retry-After`, retries transient `5xx` responses with jittered backoff, sets an explicit timeout, and fails with an actionable message when the rate limit is exhausted, including when it resets.
