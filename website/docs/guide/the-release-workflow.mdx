...
If [The changeset workflow](/guide/the-changeset-workflow) explains *why* intent-based versioning works, this page explains *how* to run it day to day -- as a solo maintainer and as a team on a pull-request flow.
...
2. **CI gates the PR** with [`molt status`](/guide/status). It fails when a versionable package changed but no changeset was added -- a prompt to run `molt add`, or `molt add --empty` if the change genuinely needs no release.
...
:::tip Steps 4 and 5 in GitHub Actions
You do not script steps 4 and 5. The [GitHub Action](/guide/ci-github-action) runs both phases on
every push to your base branch: it opens and updates the "Version Packages" pull request while
changesets are pending, and publishes to PyPI over OIDC once you merge it.
...
That guide draws the whole pull-request flow step by step. The same shape works on any forge, since
the [forge is a seam](/forges/overview).
:::

## When to use --dry-run

Every mutating command -- `add`, `version`, `publish`, `build`, `git-tag` -- can build its [plan](/guide/dry-run-and-plans) and print it *without executing*. Reach for `--dry-run` when you want to look before you leap:
...
- [Adding a changeset](/guide/adding-a-changeset) -- interactive and non-interactive `molt add`.
- [Versioning](/guide/versioning) -- everything `molt version` does, and how to preview it.
- [CI: GitHub Action](/guide/ci-github-action) -- automate steps 4 and 5.
- [Dry runs and plans](/guide/dry-run-and-plans) -- the plan object behind every command.
