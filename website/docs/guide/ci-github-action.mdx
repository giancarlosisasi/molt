...
The Action lives in [giancarlosisasi/molt-action](https://github.com/giancarlosisasi/molt-action), so you reference it as `uses: giancarlosisasi/molt-action@v1`.
...
- **Publish phase** -- there are no pending changesets but there *are* versions on disk that are not yet on PyPI. That is the state right after the release PR merges. Molt builds and [publishes](/guide/publishing) the changed packages, git-tags each one, and creates a GitHub Release per package with its changelog entry as the body.
    ...
    steps:
      - uses: actions/checkout@v5
        with:
          ...
          persist-credentials: false  # the Action brings its own push credential

      - uses: giancarlosisasi/molt-action@v1
        with:
          molt-version: "0.1.3"   # pin molt; keeps the Action and the tool in lockstep
...
|---|---|---|
| `molt-version` | latest release | The exact `molt-release` version to install, for example `"0.1.3"`. Empty installs the latest release. See [Pinning molt](#pinning-molt). |
...
You also configure PyPI Trusted Publishing once, on PyPI itself, to trust this repository and workflow. After that there is no API token anywhere in the workflow -- see [Publishing](/guide/publishing#trusted-publishing-over-oidc----no-long-lived-token).
...
- [Publishing](/guide/publishing) -- the OIDC upload flow the publish phase runs.
...
- [The changeset workflow](/guide/the-changeset-workflow) -- the add / version / publish loop this automates.
