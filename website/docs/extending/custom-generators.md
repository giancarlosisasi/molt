...
- **`changeset` and `dependencies` are read structurally.** molt has no `Changeset` or `DependencyRelease` class to import -- the generator receives plain objects: a changeset-shaped one exposing `.id`, `.summary`, `.commit` and `.front_matter`, and each dependency-shaped one exposing `.name` and `.new_version` (a parsed `packaging.version.Version`, and nothing else is guaranteed). Both positions are typed `Any` in `molt.changelog.ChangelogGenerator`, the real protocol these two methods satisfy. The dependency element's own contract is written down as `molt.changelog.DependencyReleaseLike`; annotating your parameter with it is optional and does not break the seam. `Forge`, when supplied, is real and importable from `molt.forge`.
