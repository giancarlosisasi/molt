...
class VersionSourceConfig(BaseModel):
    ...
    kind: str = Field(
        description=(
            "Where this package's version lives: 'static' ([project].version), 'file' (a version "
            "literal in a named file), 'tag' (derived from a git tag; detected but not yet "
            "releasable), or a source supplied by an installed plugin."
        ),
    )
    path: str | None = Field(
        default=None,
        description=(
            "File holding the version, relative to this package's own directory. Required for "
            "kind = 'file'."
        ),
    )
    pattern: str | None = Field(
        default=None,
        description=(
            "Regular expression locating the version inside that file. It must capture a group "
            "named 'version'; only that group is replaced when molt writes a new version."
        ),
    )
...
class Config(BaseModel):
    ...
    version_source: VersionSourceConfig | None = Field(
        default=None,
        description=(
            "Where THIS package keeps its version. Absent means molt detects it from the "
            "build-tool tables. Declared in the package's own pyproject.toml, not the "
            "workspace root's."
        ),
    )
